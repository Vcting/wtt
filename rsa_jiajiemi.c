...
int main(void)
{
    int ret;
    size_t olen = 0;//记录长度
    uint8_t out[2048/8];//一字节长类型 

    mbedtls_rsa_context ctx;    //RSA密钥结构体
    mbedtls_entropy_context entropy;//熵结构体
    mbedtls_ctr_drbg_context ctr_drbg;//随机数结构体
    const char *pers = "wtt_rsa";//用来初始化随机数的种子
    const char *msg = "This is RSA encryption and decryption!";//要加密的明文

    mbedtls_entropy_init(&entropy);//初始化熵结构体用来初始化
